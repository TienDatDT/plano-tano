...
export function EmergencyInvoiceCard({ invoice, index, onDelete, onView, onEdit, onPrint }: Props) {
  ...
  return (
    <div
      ...
      className={`
        ${color.bg} ${color.border} border rounded-2xl shadow-md w-full
        hover:shadow-lg hover:scale-[1.03] hover:rotate-0
        transition-all duration-300 cursor-pointer group flex flex-col overflow-hidden
      `}
    >
      {/* ── Header ── */}
      <div className={`${color.header} px-4 py-3 flex items-center justify-between gap-2`}>
      ...
      <div className="px-4 py-3 border-t border-neutral-200/70 flex flex-col gap-2">
        ...
        {/* Row 1: Total – full width, không bị chèn bởi buttons */}
        ...
        {/* Row 2: Actions
            • Mobile  → opacity-100 (không có hover, phải luôn thấy)
            • Desktop → opacity-0, hiện khi hover card (group-hover)
            Dùng: opacity-100 sm:opacity-0 sm:group-hover:opacity-100
        */}
